			...
			if (Input.GetMouseButtonDown (0)) {
				if (tileMouseOverScript != null) {
					...
					case Tile.OverlayType.AttackAvailable:
						...
						AttackedUnit.OnAttackSelected(AttackedUnit.Position, selectedTile.LoadedUnitScript.Position, AttackedUnit.layer, (int) selectedTile.layerNumber.x);
						selectedTile.LoadedUnitScript.OnAttackSelect(AttackedUnit.Position, selectedTile.LoadedUnitScript.Position, AttackedUnit.layer, (int) selectedTile.layerNumber.x);
						break;
					case Tile.OverlayType.MoveAvailable:
						selectedTile.LoadedUnitScript.OnMoveSelect(new Vector2(tileMouseOverScript.xcoord, tileMouseOverScript.zcoord), selectedTile.LoadedUnitScript.Position, (int) tileMouseOverScript.layerNumber.x, (int) selectedTile.layerNumber.x);
						break;
					case Tile.OverlayType.SpecialAvailable:
						selectedTile.LoadedUnitScript.OnSpecialSelect(new Vector2(tileMouseOverScript.xcoord, tileMouseOverScript.zcoord), selectedTile.LoadedUnitScript.Position, (int) tileMouseOverScript.layerNumber.x, (int) selectedTile.layerNumber.x);
						break;
					case Tile.OverlayType.BuffAvailable:
						Unit BuffedUnit = tileMouseOverScript.LoadedUnitScript;
						selectedTile.LoadedUnitScript.OnSpecialSelect(BuffedUnit.Position, selectedTile.LoadedUnitScript.Position, BuffedUnit.layer, (int) selectedTile.layerNumber.x);
						break;
					}
				}
			}
